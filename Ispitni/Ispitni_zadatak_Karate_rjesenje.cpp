...
bool ValidirajLozinku(string lozinka) {
	string pravilo = ("(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*\\W)[A-Za-z\\d\\W]{7,}");
	return regex_match(lozinka, regex(pravilo));
}
...
public:
	...
	void AddElement(const T1& el1, const T2& el2,int lokacija=-1) {
		if (_trenutno == max)
			throw exception("Maksimalan broj elemenata!");
		if (lokacija != -1 && lokacija >= 0 &&lokacija <= _trenutno) {
...
public:
	...
	*/
	bool AddOcjena(int ocjena, const Datum& datum) {
		if (_ocjene->getTrenutno() != 0 && datum - _ocjene->getElement2(_ocjene->getTrenutno() - 1) < 3) {
			return false;
		}
		else {
...
public:
	...
	- dodavanje tehnika za visi pojas ako prethodni pojas nema evidentirane najmanje 3 tehnike ili nema
	prosjecnu ocjenu svih tehnika vecu od 3.5
	(onemoguciti dodavanje tehnike za NARANDZASTI ako ne postoji najmanje 3 tehnike za ZUTI pojas ili
	njihov prosjek nije veci od 3.5)
	funkcija vraca true ili false u zavisnosti od (ne)uspjesnost izvrsenja
	*/
	void Ispis(Tehnika tehnika, Pojas pojas, Polaganje p) {
		m.lock();
		cout << "FROM:info@karate.ba" << " TO: " << GetEmail() << endl;
		cout << "Postovani, " << GetImePrezime() << ", evidentirana vam je thenika X za Y pojas. Dosadasnji uspjeh (prosjek ocjena) \nna pojasu Y iznosi F, a ukupni uspjeh(prosjek ocjena) na svim pojasevima iznosi Z." << endl;
			cout << "Pozdrav\n\nKARATE Team." << endl;
		m.unlock();
	}
